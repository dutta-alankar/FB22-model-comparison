...
#
################################################################
# runfile('/Users/alankard/coding/test/MultiphaseGalacticWind/wind-equation-my_params.py', args='True', wdir='/Users/alankard/coding/test/MultiphaseGalacticWind')
...
if len(sys.argv)==1:
    ...
    drummond = (sys.argv[1]!="False")
print(f'{"Drummond" if drummond else "our"} model')
...
def Multiphase_Wind_Evo(r, state):
    ...
    t_cool_cl    = tcool_P(T_cloud, Pressure/kb, Z_cloud/Z_solar)[()] 
    t_cool_cl    = np.where(t_cool_cl<0, 1e10*Myr, t_cool_cl)
    _vanilla = False
    if drummond:
        ksi          = r_cloud / (v_turb * t_cool_layer)
    else:
        if r>=cold_cloud_injection_radial_extent:
            ksi = r_cloud_init * (r/cold_cloud_injection_radial_extent) / (v_turb * t_cool_layer)
        else:
            ksi          = r_cloud / (v_turb * t_cool_layer)
    AreaBoost    = chi**CoolingAreaChiPower
    v_turb_cold  = v_turb * chi**ColdTurbulenceChiPower
    if drummond or _vanilla:
        alpha = 0.25 if ksi >= 1.0 else 0.50
    else:
        alpha = 0.25 if ksi >= 1.0 else 0.50
    # print(f"{r/pc:.2e}, {f_turb0 * v_rel * ksi**alpha:.2e}") #(r/v_cloud)/(chi**0.5*r_cloud_init/v_wind))
    if drummond:
        Mdot_grow    = fcool * 3.0 * M_cloud * v_turb * AreaBoost / (r_cloud * chi) * np.where( ksi < 1, ksi**0.5, ksi**0.25 )
    else:
        if _vanilla:
            vin = max(f_turb0 * v_rel * ksi**alpha, 
                      0.075*cs_cl_sq**0.5*(r_cloud_init/(cs_cl_sq**0.5*t_cool_cl))**0.25)
        else:
            # tmp = np.power(r_cloud/(np.power(cs_cl_sq,0.5)*t_cool_cl),0.25)
            vin = max( f_turb0 * v_rel * ksi**alpha, 
                      0.19*np.power(cs_cl_sq,0.5)*np.power(r_cloud/(np.power(cs_cl_sq,0.5)*t_cool_cl),0.25) )
        Mdot_grow    = 3*M_cloud*fcool/(chi**0.5*r_cloud)*vin
    Mdot_loss    = fmix * 3.0 * M_cloud * v_turb_cold / r_cloud 
...
def cloud_ksi(r, state):
    ...
    ksi          = r_cloud / (v_turb * t_cool_layer)
    if not drummond:
        ksi = np.where( r>=cold_cloud_injection_radial_extent, 
                        r_cloud_init * (r/cold_cloud_injection_radial_extent) / (v_turb * t_cool_layer),
...
eta_E            = 0.5
eta_M            = 0.2 # 0.1 
eta_M_cold       = 0.15 # 0.15 # 1.5*eta_M  # 0.15 # 0.2
log_M_cloud_init = 2.0
...
print( "v_wind = %.1e km/s  n_wind = %.1e cm^-3  P_wind = %.1e kb K cm^-3 T_wind = %.1e K" %(v0/1e5, rho0/(mu*mp), P0/kb, P0/rho0*(mu*mp/kb)))
...
# 10**2*r_sonic
sol_sup = solve_ivp(Single_Phase_Wind_Evo, [10**(np.log10(r_sonic)+dlogr0),40*kpc], np.r_[v0_sup, rho0_sup, P0_sup], 
    events=[supersonic], 
    dense_output=True,
    rtol=1e-12, atol=[1e-3, 1e-7*mp, 1e-2*kb])
...
chi_init            = (P0/rho0*(mu*mp/kb))/T_cloud
...
cold_cloud_injection_radial_extent  = 220*pc
...
v_cloud_init    = 30 * km/s  # 10**1.5 * km/s 
...
rho_cloud_init = P_init/(kb*T_cloud)*mu*mp
r_cloud_init = (3*10.**log_M_cloud_init*Msun/(4*np.pi*rho_cloud_init))**(1/3.)
tcc_init = np.sqrt(chi_init)*r_cloud_init/v_init
tdrag_init = np.sqrt(chi_init)*tcc_init
print("Initial cloud radius: %.1e pc"%(r_cloud_init/pc))
...
### integrate!
# 1e2*r_sonic
sol = solve_ivp(Multiphase_Wind_Evo, [r_init, 40*kpc], 
...
data["single_phase"] = np.vstack([ r_hot_only/kpc, v_wind_hot_only/1e5, cs_wind_hot_only/1e5, 
                                   rho_wind_hot_only/(mu*mp), P_wind_hot_only/kb, 
                                   np.ones_like(rho_wind_hot_only)*Z_wind_init/Z_solar, 
                                   K_wind_hot_only, 
                                   Mdot_wind_hot_only,
                                   Edot_wind_hot_only ]).T
...
print(f"ksi_init = {cloud_ksi(r, sol.y)[0]:.2f}")
...
data["multi_phase"] = np.vstack([ r/kpc, v_wind/1e5, cs_wind/1e5, 
                                  np.ma.masked_where(M_cloud<M_cloud_min, v_cloud)/1e5,
                                  rho_wind/(mu*mp), P_wind/kb, rhoZ_wind / rho_wind / Z_solar,
                                   np.ma.masked_where(M_cloud<M_cloud_min, Z_cloud)/Z_solar, 
                                  K_wind, 
                                  np.ma.masked_where(M_cloud<M_cloud_min, M_cloud)/Msun, 
                                  # np.ma.masked_where(M_cloud<M_cloud_min, M_cloud)[-1]/Msun, 
                                  np.ma.masked_where(M_cloud<M_cloud_min, cloud_ksi(r, sol.y)), 
                                  Mdot_wind, np.ma.masked_where(M_cloud<M_cloud_min, cloud_Mdots[0]), 
                                  Edot_wind, 
                                  np.ma.masked_where(M_cloud<M_cloud_min, cloud_Edots[0]) ]).T
...
axX.axhline(1 if drummond else 0.1, color='grey', lw=0.5, ls=':')

axK.set_ylim(ymin=1.0e-2*np.min(K_wind), ymax=1.0e2*np.max(K_wind))
axM.set_ylim(ymin=10.**-2, ymax=4e6)
axMd.set_ylim(ymin=0.08, ymax=10.**2)
...
plt.savefig(f'FB22_{"drumm" if drummond else "new-params"}.pdf', 
            dpi=200, bbox_inches='tight')
plt.show()
plt.clf()

with open(f'FB22_{"drumm" if drummond else "new-params"}_logMcold-init={log_M_cloud_init:.1f}.pickle', 'wb') as handle:
    print(f'Writing FB22_{"drumm" if drummond else "new-params"}_logMcold-init={log_M_cloud_init:.1f}.pickle')
